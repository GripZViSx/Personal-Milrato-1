  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        let button_home = new MessageButton().setStyle('STANDARD').setCustomId('2').setEmoji("🏠").setLabel(handlemsg(client.la[ls].cmds.info.help.buttons.home))
        ...
        let menuOptions = [
          ...
          },
          {
            ...
            value: "Music",
            emoji: "🎶",
            description: "Commands to play Music or add Filter"
          },
          {
            label: "Filter",
            value: "Filter",
          ...
          },
          {
            ...
            value: "Nsfw",
            emoji: "🔞",
            description: "Commands for Nsfw Content"
          },
          {
            label: "Customcommand",
            value: "Customcommand",
        ...
        let menuSelection = new MessageSelectMenu()
          ...
          .setPlaceholder("Click me to view Pages")
          .setMinValues(1)
          .setMaxValues(5)
        ...
        .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
        .setFooter("This Bot Is Only For Test")
        .setTitle(`Info ${client.user.username}__`)
        .addField("**__My Features__**",
`>>> __**More Than 58 Systems**__ **Twitter**ㅤㅤㅤㅤㅤ**Youtube-Auto-Poster**\n**Application**ㅤㅤㅤㅤㅤ**Ticket System**\n**Welcome-Images**ㅤㅤㅤㅤㅤ**Reaction Role System**\n**Advanced Music System**ㅤㅤㅤㅤㅤ**Minigames**\n**Fun**ㅤㅤㅤㅤㅤ**Administration**`)
.addField(":chart_with_upwards_trend: **__STATS__**",
`>>> **Commands Active : ${client.commands.map(a=>a).length}**\n**Guilds : ${client.guilds.cache.size}**\n**Uptime : ${duration(client.uptime).map(i=> `\`${i}\``).join("︲")}**\n**Ping : \`${Math.floor(client.ws.ping)}ms\`**`)
        let helpmsg = await message.reply({   
            content: `Browse Categories And Commands`,
            embeds: [OverviewEmbed], 
            components: allbuttons
        }).catch(e=>{
          console.log(e.stack ? String(e.stack).grey : String(e).grey)
          return message.reply(`<a:crossmark:1024362593621651476> There Was Some Error Sending This Message !\n**__Often Causes__**\nMissing \`EMBED_LINKS\` permission\n\`Problem With\` Some \`Programming Of Bot\``).catch(() => {})
        });
        var edited = false;
